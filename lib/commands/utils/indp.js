...
async function indp(mf) {
    for (let i = 0; i < Object.keys(mf.dependencies).length; i++) {
        ...
        await pacote.manifest(`${Object.keys(mf.dependencies)[i]}@${version}`).then(async (manifest) => {
            await pacote.tarball.file(manifest.dist.tarball, `./cache/${Object.keys(mf.dependencies)[i]}.tgz`).then(async () => {
                await pacote.extract(`./cache/${Object.keys(mf.dependencies)[i]}.tgz`, `./cache/${Object.keys(mf.dependencies)[i]}`).then(async () => {
                    fs.unlinkSync(`./cache/${Object.keys(mf.dependencies)[i]}.tgz`)
